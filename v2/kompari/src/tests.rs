...
use crate::dirdiff::{DirDiffConfig, LeftRightError};
use crate::imgdiff::ImageDifference;
use std::path::Path;
...
pub fn test_compare_dir() {
    ...
    assert!(matches!(
        res[1].image_diff,
        Err(LeftRightError::Left(crate::Error::FileNotFound(_)))
    ));
    assert!(matches!(
        res[2].image_diff,
        Err(LeftRightError::Right(crate::Error::FileNotFound(_)))
    ));
    assert!(matches!(
        res[3].image_diff,
